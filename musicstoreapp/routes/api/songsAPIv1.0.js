...
const {songValidatorUpdate} = require("./songValidatorUpdate");
...
module.exports = function (app, songsRepository, usersRepository) {
    ...
    app.delete('/api/v1.0/songs/:id', function (req, res) {
        try {
            ...
            isUserAuthor(res.user, songId).then(isAuthor => {
                if (isAuthor) {
    ...
    async function isUserAuthor(user, songId) {

        let filterSongAuthor = {$and: [{"_id": songId}, {"author": user}]};
        let options = {};
        try {
            const songs = await  songsRepository.getSongs(filterSongAuthor, options);
            if (songs.length > 0) {
                return true;
            } else {
                return false;
